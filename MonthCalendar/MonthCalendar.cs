namespace CustomControls
{
   ...
   public class SphMonthCalendar : Control
   {
      #region fields

	   private DateTime userSelectedDate_ = DateTime.MinValue;
         ...
         if (e.Button == MouseButtons.Left)
         {
            ...
            switch (hit.Type)
            {
				case MonthCalendarHitType.Day:
		            userSelectedDate_ = DateTime.MinValue;
		            foreach (var range in selectionRanges)
		            {
			            if (range.Contains(hit.Date))
			            {
							userSelectedDate_ = hit.Date;
							break;
			            }
		            }
					break;
		      ...
		      switch (hit.Type)
		      {
			      case MonthCalendarHitType.Day:
			      {
				      ...
				      var bold = this.GetBoldedDates().Contains(hit.Date) ||
				                 this.boldDatesCollection.Exists(d => d.Value.Date == hit.Date.Date);
