namespace CustomControls
{
   ...
   public class MonthCalendarRenderer : MonthCalendarAbstractRenderer
   {
      ...
      /// </summary>
	   private readonly SphMonthCalendar monthCal;
	  ...
	  public MonthCalendarRenderer(SphMonthCalendar calendar)
         : base(calendar)
      {
      ...
      /// </summary>
	  public SphMonthCalendar MonthCalendar
      {
         ...
         MonthCalendarHeaderState state)
      {
	   ...
	   public override void DrawDay(Graphics g, MonthCalendarDay day)
	   {
		   ...
		   else if (!boldDate.IsEmpty && boldDate.Category.BackColorStart != Color.Empty &&
		            boldDate.Category.BackColorStart != Color.Transparent)
		   {
			   FillBackground(
				   g,
				   rect,
				   boldDate.Category.BackColorStart,
				   boldDate.Category.BackColorEnd.IsEmpty || boldDate.Category.BackColorEnd == Color.Transparent
					   ? boldDate.Category.BackColorStart
					   : boldDate.Category.BackColorEnd,
		   ...
		   using (StringFormat format = GetStringAlignment(this.monthCal.DayTextAlignment))
		   {
			   Color textColor = bold
				                     ...
				                     : (day.Selected
					                        ...
					                        : (day.MouseOver
						                           ? colors.DayActiveText
						                           : (day.TrailingDate
							                              ? colors.DayTrailingText
							                              : colors.DayText)));
			   ...
			   using (SolidBrush brush = new SolidBrush(textColor))
			   {
					   ...
					   FontStyle.Bold))
				   {
					   ...
					   string dayString = this.monthCal.UseNativeDigits
						                      ? DateMethods.GetNativeNumberString(calDate.Day,
		   ...
		   //DateTime dtBorder = DateTime.Today; // my code
		   //if (monthCal.UserSelectedDate != DateTime.MinValue)
		   //dtBorder = monthCal.UserSelectedDate;
		   if (day.Date == monthCal.UserSelectedDate /*DateTime.Today*/)
		   {
			   rect.Height -= 1;
			   rect.Width -= 2;
			   Color borderColor = Color.Brown;
			   //Color borderColor = day.Selected
			   //             ? colors.DaySelectedTodayCircleBorder
			   //             : (day.MouseOver ? colors.DayActiveTodayCircleBorder 
			   //                     : colors.DayTodayCircleBorder);
